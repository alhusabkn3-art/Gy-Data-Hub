...
// ── Backend ledger response compatibility ─────────────────────────────────────
...
function normalizeWalletLedgerResponse(
  ...
  const pages = Number.isFinite(pagesValue)
    ? Math.max(1, pagesValue)
    : 1;
...
function WalletModal({
  ...
  const inputValid =
    amountNum >= 1 && reason.trim().length >= 10;
      ...
      <div className="bg-[#0D1F38] border border-border rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-5">
          ...
          {step === 'confirm' && (
            <div className="space-y-4">
              <div
                className={`border rounded-xl p-4 space-y-2 bg-white/[0.03] ${accentClass}`}
              >
              ...
              <p className="text-xs text-amber-400/80 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2">
                ⚠️ This action will be logged in the audit
                trail.
              </p>
...
export default function WalletManagement() {
      ...
      try {
        ...
        const normalized =
          normalizeWalletLedgerResponse(
            ledgerData as unknown as WalletLedgerApiResponse,
          );
         ...
         *   pages
         *
         * normalizeWalletLedgerResponse() supports BOTH,
         * so Wallet Management will not crash regardless
         * of which response shape the server returns.
         */
        setLedger(normalized.entries);
        setLedgerTotal(normalized.total);
        setLedgerPages(normalized.pages);
        setLedgerPage(page);
      } catch (err: unknown) {
        const msg =
          err instanceof Error
        ...
        /*
         * Never allow failed/malformed API data to leave
         * ledger as undefined and crash the React render.
         */
        setLedger([]);
        setLedgerTotal(0);
        setLedgerPages(1);
  ...
  const loadLedgerPage = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        const normalized =
          normalizeWalletLedgerResponse(
            data as unknown as WalletLedgerApiResponse,
          );
  ...
  useEffect(() => {
    if (selectedUser) {
      setLedgerPage(1);
      void loadWallet(selectedUser, 1);
    }
    ...
    <div className="min-h-screen bg-[#0A1628] text-white p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-400">
            Credit or debit user wallets with full audit
            trail
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-[#0A1628] overflow-y-auto">
          ...
          <div className="sticky top-0 z-10 bg-[#0A1628]/95 backdrop-blur-sm border-b border-border px-6 py-4 flex items-center justify-between">
            ...
            <button
              ...
              className="p-2 rounded-lg hover:bg-white/[0.07] text-zinc-400 hover:text-white transition-colors"
            >
              <X size={18} />
            </button>
          </div>
                ...
                <>
                  ...
                  <div className="bg-card border border-border rounded-2xl p-4">
                    ...
                    <p className="text-xl font-bold text-amber-400">
                      {fmtNaira(
                        walletSummary.totalReversed,
                      )}
                    </p>
                  </div>
                </>
              ) : (
                <div className="col-span-full bg-card border border-border rounded-2xl p-6 text-center text-sm text-zinc-500">
