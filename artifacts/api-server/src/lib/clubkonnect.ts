...
export async function getDataPlans(
   ...
   * returns the complete catalogue as:
   *
   * {
   *   "MOBILE_NETWORK": {
   *     "MTN": [
   ...
   * }
   *
   * It is NOT necessary to send UserID/APIKey,
   * MobileNetwork or MobileNumber here.
   *
   * We fetch the catalogue and select the
   * requested network locally.
   */
  ...
  for (const product of products) {
     ...
     * PRODUCT_ID   = 1000
     *
     * ClubKonnect purchase API uses:
     * DataPlan=1000
     *
     * Therefore we MUST use PRODUCT_ID,
     * not PRODUCT_CODE.
     */
...
export async function purchaseData(
  ...
  /*
   * IMPORTANT:
   * The purchase API must receive the actual recipient number.
   *
  ...
  const url = buildUrl(
    'APIDatabundleV1.asp',
    {
       ...
       * Always send the validated server-side
       * mobileNumber, not the raw params.phone.
       */
      MobileNumber:
        mobileNumber,
