...
export default function AdminDashboard({ onNavigate }: AdminDashboardProps) {
  ...
  // Normalize admin statistics so an incomplete/cached API response
  // can never crash the dashboard when a numeric field is missing.
  const safeStats = stats
    ? {
        ...
        pendingKycUsers: Number(stats.pendingKycUsers ?? 0),
        unverifiedUsers: Number(stats.unverifiedUsers ?? 0),
        ...
        totalWalletBalance: Number(stats.totalWalletBalance ?? 0),
        avgTransactionValue: Number(stats.avgTransactionValue ?? 0),
      }
    : null;

  const maxRevenue =
    weeklyRevenue.length > 0
      ? Math.max(...weeklyRevenue.map(d => d.amount), 1)
      : 1;
    ...
    <div className="p-4 lg:p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          ...
          sub={
            safeStats
              ? `${safeStats.activeUsers.toLocaleString()} active`
              : undefined
          }
          icon={Users}
        ...
        <StatCard
          ...
          value={
            safeStats
              ? fmtNaira(safeStats.totalRevenue)
              : '—'
          }
          sub={
            safeStats
              ? `${fmtNaira(safeStats.todayRevenue)} today`
              : undefined
          }
          icon={TrendingUp}
        ...
        <StatCard
          ...
          value={
            safeStats
              ? safeStats.pendingTransactions.toString()
              : '—'
          }
          sub={
            safeStats
              ? `${safeStats.failedTransactions} failed`
              : undefined
          }
          icon={Clock}
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatusCard
          ...
          value={safeStats?.successfulTransactions?.toLocaleString()}
          label="Successful"
          loading={isLoading && !stats}
          onClick={() => onNavigate('transactions')}
        />
        ...
        <StatusCard
          ...
          value={safeStats?.pendingTransactions}
          label="Pending"
          loading={isLoading && !stats}
          onClick={() => onNavigate('transactions')}
        />
        ...
        <StatusCard
          ...
          value={safeStats?.failedTransactions}
          label="Failed"
          loading={isLoading && !stats}
          onClick={() => onNavigate('transactions')}
        />
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <ChartCard
          ...
          sub={
            safeStats
              ? `${fmtNaira(safeStats.weekRevenue)} this week`
              : undefined
          }
          onClick={() => onNavigate('wallet')}
        >
            ...
            },
            {
              ...
              value:
                isLoading && !stats
                  ...
                  : (
                      safeStats
                        ? safeStats.suspendedUsers +
                          safeStats.unverifiedUsers
                        : '—'
                    ),
            },
          ].map(
      ...
      {extData &&
        extData.recentActivity.length > 0 && (
          <div className="bg-[#0B1B35] border border-white/[0.07] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Activity className="w-4 h-4 text-white/40" />
