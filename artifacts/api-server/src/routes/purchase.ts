...
async function findDataPricingRule(
  ...
  const exact = await db.execute<PricingRule>(sql`
    SELECT
      id,
      service_type,
      provider,
      ...
      selling_price,
      enabled,
      COALESCE(cashback_enabled, false) AS cashback_enabled,
      COALESCE(cashback_type, 'percentage') AS cashback_type,
      COALESCE(cashback_value, 0) AS cashback_value
    FROM pricing_rules
  ...
  const fallback = await db.execute<PricingRule>(sql`
    SELECT
      id,
      service_type,
      provider,
...
async function creditCashback(
  ...
  if (wallet.rows.length === 0) {
    await db.execute(sql`
      ...
      DO UPDATE SET
        balance =
          cashback_wallets.balance + EXCLUDED.balance,
        updated_at = NOW()
    `);
  } else {
    await db.execute(sql`
  ...
  return safeNumber(updated.rows[0]?.balance);
}
...
export async function transferCashbackToMain(
  ...
  const newCashbackBalance =
    safeNumber(cashbackResult.rows[0].balance);

  const newMainBalance =
    safeNumber(mainResult.rows[0].balance);
  ...
  try {
    await db.execute(sql`
      ...
      VALUES (
        ${userId}::uuid,
        ${cashbackResult.rows[0].id}::uuid,
        ${amount},
        ${newCashbackBalance + amount},
        ${newCashbackBalance},
        ${mode}
      )
    `);
...
router.post(
  ...
  async (
    ...
    if (!/^\d{4,6}$/.test(purchasePin)) {
      res.status(400).json({
        success: false,
        error: 'Purchase PIN is required.',
    ...
    /*
     * Existing accounts may not have purchase_pin_hash yet.
     * For those accounts the login PIN is temporarily accepted
     * as the purchase PIN until a separate purchase PIN is set.
     */
    const pinHash =
      pinUser.purchase_pin_hash ||
      pinUser.login_pin_hash;
    ...
    try {
      ...
      if (
        ...
        await notifyUser(
          ...
          `${planName || 'Data bundle'} has been purchased successfully for ${phone}.`,
        );
      ...
      if (
        ...
        await notifyUser(
          ...
          `Your ${planName || 'data purchase'} for ${phone} is being processed.`,
        );
