...
export const AdminProvider = ({
  ...
  const fetchUsers = async (
    ...
    try {
      ...
      /*
       * The current backend /api/admin/users
       * does not filter by KYC. Do not send
       * a fake KYC filter and pretend it works.
       */
      if (
  ...
  /*
   * IMPORTANT:
   * The backend calls these accounts "admins",
   * not "accounts".
   *
   * Correct routes:
  ...
  const addAdminAccount =
    async (data: {
      ...
      try {
         ...
         *   super_admin
         *
         * Customer Care is a frontend role only
         * in the current admin schema, so never
  ...
  const refreshAll =
    async (): Promise<void> => {
      ...
      /*
       * /api/admin/admins is Super Admin only.
       * Do not call it for ordinary admins/customer care.
       */
      if (isSuperAdmin) {
        tasks.push(
          fetchAdminAccounts(),
  ...
  /*
   * Load Super Admin data after the session
   * has been restored.
   */
  useEffect(() => {
    if (
