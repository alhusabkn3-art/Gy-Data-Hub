 ...
 *
 * Airtime provider:
 *   ClubKonnect (intentionally unchanged for now)
 */
...
/* ─────────────────────────────────────────────────────────────
   AIRTIME PURCHASE
   CLUBKONNECT — LEFT UNCHANGED FOR NOW
───────────────────────────────────────────────────────────── */
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!smedata.isSmeDataNetwork(network)) {
      res.status(400).json({
        error:
          'Invalid network for SMEDATA. Use: mtn, glo, or airtel.',
      });
      return;
    }
    ...
    let vendorResult: smedata.SmePurchaseResult = {
      code: 'failure',
      message: 'SMEDATA purchase was not attempted.',
    };

    try {
      vendorResult = await smedata.purchaseData({
        network,
        phone: cleanPhone,
        planCode,
        requestId,
    ...
    const normalizedStatus =
      smedata.normalizeSMEStatus(vendorResult.code);

    const providerRef =
      smedata.getSMEProviderRef(vendorResult);
    ...
    logger.info(
      {
        ...
        requestId,
        normalizedStatus,
        vendorStatus: vendorResult.code,
    ...
    res.status(422).json({
      ...
      vendorStatus: vendorResult.code,
      error:
        `SMEDATA returned: ${vendorResult.code || 'failure'} — ` +
        `${vendorResult.message || 'No message'}`,
    });
  },
);
