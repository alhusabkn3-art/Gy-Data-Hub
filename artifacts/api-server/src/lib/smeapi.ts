 ...
 * then filter them locally by network.
 */
export async function getDataPlans(
  ...
  for (
    ...
    /*
     * If the provider gives a network field,
     * filter it strictly.
     *
     * If the provider omits the network field,
     * keep the plan because /api/dataplans/
  ...
  /*
   * Log a small safe sample of the
   * provider plan IDs.
   *
