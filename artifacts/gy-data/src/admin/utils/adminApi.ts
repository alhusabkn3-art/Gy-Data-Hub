...
export async function apiCreditWallet(
  ...
  const r = await adminApi(
    `/api/admin/users/${userId}/wallet/credit`,
    {
      method: 'POST',
...
export async function apiGetNotificationHistory(
  ...
  const r = await adminApi(
    `/api/admin/notifications/history?page=${page}&limit=50`
  );
...
export async function apiGetDashboardExtended(): Promise<DashboardExtended> {
  ...
  return {
    dailyRevenue: Array.isArray(data.daily)
      ? data.daily.map((item) => ({
          day: String(item.day ?? ''),
...
export async function apiUpdateApiConfig(
  ...
  const r = await adminApi(
    `/api/admin/api-management/configs/${key}`,
    {
      method: 'PATCH',
      body: JSON.stringify(data)
    }
