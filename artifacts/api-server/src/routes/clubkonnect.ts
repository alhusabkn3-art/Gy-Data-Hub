...
function looksLikeProduct(
  ...
  const code = pickString(value, CODE_KEYS);
  const name = pickString(value, NAME_KEYS);
...
function collectProductObjects(
  ...
  for (const [key, child] of Object.entries(object)) {
    if (PRODUCT_CONTAINER_KEYS.includes(key)) {
      continue;
    }
...
export async function getDataPlans(
   ...
   * IMPORTANT:
   *
   * APIDatabundlePlansV2.asp is the ClubKonnect
   * endpoint for retrieving available databundle
   * plans.
   *
   * It does NOT require MobileNumber.
   *
   * The old implementation used:
   *
   * APIDatabundlePlansV1.asp
   *
   * together with MobileNumber, which caused ClubKonnect
   * to return:
   *
   * {"status":"MISSING_PHONE_NUMBER"}
   *
   * The phone number is only needed later when the
   * selected plan is purchased through
   * APIDatabundleV1.asp.
   */
    ...
    logger.warn(
      {
        ...
        rootKeys: Object.keys(root),
        responsePreview:
          JSON.stringify(root).slice(0, 6000),
      },
