...
async function buildAll() {
  ...
  await esbuild({
    ...
    // Make sure packages that are cjs only (e.g. express)
...
globalThis.__dirname = __bannerPath.dirname(globalThis.__filename);
  ...
  // connect-pg-simple needs its table.sql file at runtime.
  // Copy it into dist so Render can create the session table.
  const require = createRequire(import.meta.url);
  const connectPgSimpleDir = path.dirname(
    require.resolve("connect-pg-simple")
  );
