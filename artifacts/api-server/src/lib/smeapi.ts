...
/**
 * SME API accepts both:
 *   https://smeapi.com.ng
 *   https://smeapi.com.ng/api/
 *
 * Normalize both forms to:
 *   https://smeapi.com.ng/api
 */
...
const BASE_URL = (() => {
  const clean = configuredBaseUrl.replace(/\/+$/u, '');
...
export async function requeryTransactionByRef(
  ...
  try {
    ...
    logger.info(
      ...
      'SME API transaction status requery',
    );
