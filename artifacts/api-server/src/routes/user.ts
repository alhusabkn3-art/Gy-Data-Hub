...
//
// Wallet funding must happen through the verified payment flow, where the
// payment provider confirms that the money was actually received before the
// wallet is credited.
//
// This endpoint is intentionally kept as a route so old clients do not
// receive a generic 404. It returns 410 and performs NO wallet mutation.
router.post('/wallet/fund', async (_req: Request, res: Response): Promise<void> => {
  res.status(410).json({
    error: 'Direct wallet funding is disabled. Complete a verified payment to fund your wallet.',
  });
});
...
router.post('/transactions', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!type || !service || !provider || amount === undefined) {
    res.status(400).json({
      error: 'type, service, provider, and amount are required.',
    });
    return;
  }
...
router.put('/preferences', async (req: Request, res: Response): Promise<void> => {
  ...
  if (existing) {
    ...
    const [updated] = await db
      ...
      .where(eq(userPreferencesTable.userId, userId))
      .returning({
        preferences: userPreferencesTable.preferences,
      });
  ...
  if (user.usernameChangedAt) {
    ...
    if (elapsed < thirtyDaysMs) {
      const nextChangeAt = new Date(
        user.usernameChangedAt.getTime() + thirtyDaysMs,
      ).toISOString();
...
router.put('/pin', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!/^\d{6}$/.test(newPin)) {
    res.status(400).json({
      error: 'newPin must be exactly 6 digits.',
    });
    return;
  }
