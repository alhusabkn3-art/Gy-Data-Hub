...
export default function BuyAirtimeScreen() {
  ...
  /*
   * Keep the same idempotency key while retrying
   * the same purchase intent.
   *
   * Reset when network, phone, or amount changes.
   */
   ...
   * First stage:
   *
   * Validate the purchase and open the Purchase PIN
   * modal. We do NOT send the transaction yet.
   */
  ...
  /*
   * Actual Airtime purchase.
   *
   * This directly calls the backend route because the
   * current AppContext purchaseAirtime method in the ZIP
   * does not accept/pass purchasePin.
   */
  const handlePurchaseWithPin =
    async (
      ...
      /*
       * Backend currently requires 4-6 digits.
       * We use exactly 6 digits for the separate
       * Purchase PIN flow.
       */
      if (
        !/^\d{6}$/.test(pin)
      ...
      try {
        ...
        if (
           ...
           *
           * If the user retries the same intent, the same
           * transaction key is reused instead of creating
           * another debit.
           */
          return;
        }
         ...
         * Failed transaction.
         *
         * The backend refunds the wallet for provider
         * failures before returning failed.
         */
        if (
          result.success !==
          true
        ...
        /*
         * Refresh both wallet balances so the UI
          ...
          >
            ...
            >
              ...
              <input
                ...
                placeholder="6-digit Purchase PIN"
