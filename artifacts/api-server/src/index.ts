 ...
 *
 * This is intentionally awaited before the HTTP server starts so that
 * Render Free can initialize the required tables without SSH/Shell access.
 *
 * If the bootstrap fails, the error is logged and the normal application
 * startup continues. This prevents a temporary database/bootstrap problem
 * from hiding the actual HTTP service.
 */
try {
  ...
  logger.error(
    { err },
    'Database bootstrap failed — continuing server startup',
  );
}
...
async function resolveStuckTransaction(
  ...
  if (ckAvailable && tx.reference) {
    try {
      ...
      resolvedStatus = normalizeCKStatus(
        vendorStatus.status,
      );
      ...
      logger.info(
        {
          txId: tx.id,
          reference: tx.reference,
          vendorStatus: vendorStatus.status,
          resolvedStatus,
        },
  ...
  await db.transaction(async (trx) => {
    const walletResult = await trx.execute<{
      balance: string;
      id: string;
    }>(
      sql`
  ...
  try {
    ...
    httpServer.listen(
      ...
      () => {
        ...
        // Database recovery must NEVER prevent
