...
/**
 * Returns a provider URL suitable for logging.
 *
 * API credentials are deliberately removed before logging.
 */
function getSafeProviderUrl(
  rawUrl: string,
): string {
  try {
...
export async function purchaseData(
   ...
   *
   * This verifies what the server is actually preparing
   * to send to ClubKonnect without exposing credentials.
   */
  try {
    const safeUrl =
    ...
    logger.error(
      ...
      'Unable to inspect ClubKonnect outgoing data purchase URL',
    );
  }
  ...
  /*
   * Log the actual HTTP response URL safely.
   *
   * Credentials are removed before logging.
   */
  logger.info(
    {
      endpoint:
