...
router.post('/register', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!name || !phone || !email || !loginPin || !username) {
    res.status(400).json({
      error: 'name, phone, email, loginPin, and username are required.',
    });
    return;
  }
  ...
  if (!/^[a-z0-9]{4,15}$/.test(normalizedUsername)) {
    res.status(400).json({
      error: 'username must be 4–15 characters (letters and numbers only, no symbols).',
    });
    return;
  }
  ...
  if (normalizedPhone.length < 10 || normalizedPhone.length > 11) {
    res.status(400).json({
      error: 'Please enter a valid Nigerian phone number (10–11 digits).',
    });
    return;
  }
  ...
  try {
    ...
    logger.error(
      { err },
      'Session regeneration failed on register'
    );
...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
  const { phone, loginPin } = req.body as {
    phone?: string;
    loginPin?: string;
  };
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [user] = await db
      .select({
        id: usersTable.id,
        resetOtpHash: usersTable.resetOtpHash,
        resetOtpExpiry: usersTable.resetOtpExpiry,
      })
      .from(usersTable)
      .where(eq(usersTable.phone, normalizedPhone));
