...
export async function getDataPlans(
  ...
  for (const key of networkKeys) {
    if (root[key] !== undefined) {
      networkSources.push(root[key]);
    }
  }

  if (networkSources.length === 0) {
    const wanted = normalizedNetwork === '9mobile'
      ? '9mobile'
      : normalizedNetwork;
  ...
  if (networkSources.length === 0) {
    logger.warn(
      {
        network: normalizedNetwork,
        availableKeys: Object.keys(root),
        responsePreview:
  ...
  logger.info(
    {
      ...
      matchedNetworkKeys: networkKeys,
      rawProductCount:
        rawProducts.length,
    },
    ...
    logger.warn(
      {
        network: normalizedNetwork,
        responsePreview:
          JSON.stringify(networkSources).slice(
            0,
            4000,
          ),
      },
