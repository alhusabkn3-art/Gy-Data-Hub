...
import {
  Tags, Save, Plus, Trash2, RefreshCw,
...
function PricingRow({
  ...
  const currentCost = draft
    ? parseFloat(draft.costPrice) || 0
    : rule.costPrice;

  const currentSelling = draft
    ? parseFloat(draft.sellingPrice) || 0
    : rule.sellingPrice;

  const currentEnabled = draft
    ? draft.enabled
    : rule.enabled;
    ...
    >
      ...
      <td className="px-4 py-3 text-sm">
        <input
          ...
          onChange={e =>
            handleSellingChange(e.target.value)
          }
          className="bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-sm text-white w-24 focus:outline-none focus:border-primary/50"
          min="0"
        />
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 text-muted-foreground hover:text-white flex items-center justify-center transition-colors disabled:opacity-30"
          >
...
export default function PricingManagement() {
  ...
  const [drafts, setDrafts] =
    useState<Record<string, DraftRow>>({});
  ...
  const loadRules = useCallback(
    async (tab: ServiceTab) => {
      setLoading(true);
      setDrafts({});

      try {
        const { rules: data } =
          await apiGetPricing(tab);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Manage service pricing, markups &amp; plans
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 text-amber-400 rounded-xl transition-colors disabled:opacity-50"
            >
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-white/[0.03]">
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground border-b border-white/[0.06]">
                      Network
                    </th>
