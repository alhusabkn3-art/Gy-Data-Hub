...
async function applyCashbackIfEligible(opts: {
  ...
  try {
    getIo().to(`user:${opts.userId}`).emit('cashback:updated', { cashbackBalance });
  } catch { /* non-fatal */ }
  ...
  logger.info(
    { userId: opts.userId, cashbackAmount, planCode: opts.planCode, cashbackRef },
...
> {
  ...
  try {
    ...
    if (!rule) {
      ...
      logger.warn(
        { planCode, network },
        'No pricing rule found for plan — blocking purchase'
      );

      return {
        valid: false,
        error: 'This data plan is not currently configured. Please contact support.'
      };
    }
    ...
    // FAIL CLOSED — DB error = block purchase
    logger.error(
      { err, planCode },
      'Price validation DB lookup failed — blocking purchase (fail-closed)'
    );

    return {
      valid: false,
      error: 'Price verification is temporarily unavailable. Please try again.'
    };
  }
}
...
router.post('/airtime', async (req: Request, res: Response): Promise<void> => {
  ...
  const requestId =
    idempotencyKey ||
    `GY-AIR-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
...
router.post('/data', async (req: Request, res: Response): Promise<void> => {
  ...
  const requestId =
    idempotencyKey ||
    `GY-DAT-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
