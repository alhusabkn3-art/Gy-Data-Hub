...
export async function getDataPlans(
  ...
  const responseKeys =
    NETWORK_RESPONSE_KEYS[normalizedNetwork];
  ...
  logger.info(
    {
      ...
      networkCode,
      endpoint: 'APIDatabundlePlansV2.asp',
    },
  ...
  for (const key of responseKeys) {
    if (mobileNetwork[key] !== undefined) {
      networkEntry =
        mobileNetwork[key];
      break;
    }
  ...
  for (const product of products) {
    ...
    const productAmount =
      stringValue(product['PRODUCT_AMOUNT']);
...
export async function purchaseData(
   ...
   * IMPORTANT:
   *
   * ClubKonnect expects MobileNumber for the
   * data bundle purchase endpoint.
   *
   * Do NOT replace this with "phone", "phoneNumber",
   * "PhoneNo", etc.
   */
  const url = buildUrl(
    'APIDatabundleV1.asp',
  ...
  /*
   * Log the exact request parameters, but NEVER
   * log UserID/APIKey.
   */
  logger.info(
    {
   ...
   *
   * Keep the vendor response exactly as returned.
   * This allows us to distinguish an HTTP error,
   * invalid JSON, or a vendor-level MISSING_PHONE_NUMBER.
   */
  logger.info(
    {
      endpoint:
    ...
    'ClubKonnect raw data purchase response',
  );
...
export async function getTransactionStatus(
  ...
  if (!cleanRequestId) {
    throw new Error(
      'ClubKonnect status check requires a RequestID.',
    );
  }
