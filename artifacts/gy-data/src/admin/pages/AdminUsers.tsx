...
// Runtime-safe display helpers. Admin API data can contain null/undefined
// values even when the TypeScript contract declares strings.
      ...
      <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 bg-[#0A1628] border border-border rounded-2xl z-[60] p-5 max-w-md mx-auto shadow-2xl">
        ...
        <div className="flex items-center gap-2.5 p-3 bg-primary/5 border border-primary/15 rounded-xl mb-4">
          ...
          <div className="min-w-0">
            <p className="text-xs font-semibold truncate">{safeText(user.name, 'Unknown User')}</p>
            <p className="text-[10px] text-muted-foreground">{safeText(user.phone)}</p>
          </div>
        </div>
...
function Skeleton({ className }: { className?: string }) {
  return (
    <div className={`animate-pulse bg-white/[0.07] rounded-lg ${className ?? ''}`} />
  );
}
...
function UserDetailModal({
  ...
  const handlePinReset = async (type: 'login' | 'purchase') => {
    ...
    try {
      ...
      toast.success(
        `${type === 'login' ? 'Login' : 'Purchase'} PIN reset successfully.`,
      );
    } catch (err) {
      toast.error(`Failed: ${(err as Error).message}`);
      setPinModal(null);
    ...
    <div className="space-y-4">
      <div className="space-y-0">
        ...
        <DetailRow
          icon={CreditCard}
          label="Bank"
          value={`${safeText(user.bankName)} · ${safeText(user.accountNumber)}`}
        />
        ...
        <DetailRow
          icon={ShoppingBag}
          label="Transactions"
          value={`${safeNumber(user.transactionCount)} txns · ₦${safeNumber(user.totalSpent).toLocaleString()} spent`}
        />
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-background border border-border rounded-xl p-3 text-center">
              <p className="text-lg font-bold">
                ₦{safeNumber(walletSummary.balance).toLocaleString()}
              </p>
            ...
            <div className="bg-background border border-border rounded-xl p-3 text-center">
              <p className="text-lg font-bold">
                ₦{safeNumber(walletSummary.totalCredit).toLocaleString()}
              </p>
              <p className="text-[10px] text-muted-foreground">
                Total Credit
              </p>
            ...
            <div className="bg-background border border-border rounded-xl p-3 text-center">
              <p className="text-lg font-bold">
                ₦{safeNumber(walletSummary.totalDebit).toLocaleString()}
              </p>
              <p className="text-[10px] text-muted-foreground">
                Total Debit
              </p>
            ...
            <div className="bg-background border border-border rounded-xl p-3 text-center">
              <p className="text-lg font-bold">
                {safeNumber(walletSummary.transactionCount).toLocaleString()}
              </p>
              <p className="text-[10px] text-muted-foreground">
                Transactions
              </p>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <LedgerTypePill type={safeText(entry.type, 'unknown')} />
                      <span className="text-[10px] text-muted-foreground">
                        {safeText(entry.reference)}
                      </span>
                    </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      ₦{safeNumber(entry.balanceAfter).toLocaleString()} bal.
                    </p>
                  </div>
                </div>
              ))}
          ...
          {ledgerPages > 1 && (
            <div className="px-3 py-2 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
              <button
                ...
                onClick={() => void goLedgerPage(ledgerPage - 1)}
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          {['all', 'success', 'pending', 'failed'].map(filter => (
            <button
              key={filter}
              onClick={() => handleTxnFilter(filter)}
            ...
            {txns.map(tx => (
              ...
              >
                <div className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-base flex-shrink-0">
                  {getTxnEmoji(safeText(tx.type))}
                </div>

                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold truncate">
                      {safeText(tx.service, 'Transaction')}
                    </p>
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-bold">
                    ₦{safeNumber(tx.amount).toLocaleString()}
                  </p>

                  <p className="text-[10px] text-muted-foreground">
                    {safeText(tx.reference)}
                  </p>
                </div>
              </div>
            ))}
        ...
        {txnPages > 1 && (
          <div className="flex items-center justify-between text-xs text-muted-foreground">
            <button
              ...
              onClick={() => void fetchTxns(txnPage - 1, txnFilter)}
              className="flex items-center gap-1 px-3 py-1.5 bg-card border border-border rounded-lg disabled:opacity-40 hover:bg-white/5 transition-colors"
            >
              <ChevronLeft className="w-3.5 h-3.5" />
              Prev
            ...
            <div className="flex items-center justify-between text-[10px] text-muted-foreground/60">
              <span>
                {entry.performedByName
                  ? `by ${safeText(entry.performedByName)}`
                  : 'System'}
              </span>
                      ...
                      {loginHistory.map(h => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 text-xs text-muted-foreground max-w-[160px] truncate">
                            {safeText(h.userAgent, '').slice(0, 50) || '—'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {pinResult ? (
              <div className="space-y-4">
                <p className="font-bold text-sm">
                  {pinModal === 'login'
                    ? 'Login'
                    : 'Purchase'} PIN Reset Successful
                </p>

                <p className="text-xs text-muted-foreground">
                  Share this temporary PIN with the user. They should change it immediately after login.
                </p>
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  <strong>{safeText(user.name, 'this user')}</strong>.
                  {' '}The current PIN will be invalidated immediately.
                </p>
...
export default function AdminUsers() {
  ...
  const filtered = users.filter(u => {
    const name = safeText(u.name, 'Unknown User');
  ...
  const handleSuspend = async (u: AdminUser) => {
    ...
    if (ok) {
      toast.success(
        `${safeText(u.name, 'User')} has been suspended.`,
      );
  ...
  const handleActivate = async (u: AdminUser) => {
    ...
    if (ok) {
      toast.success(
        `${safeText(u.name, 'User')} has been activated.`,
      );
      setSelectedUser(null);
    } else {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {usersLoading
              ? 'Loading…'
              : `${safeNumber(usersTotal).toLocaleString()} registered users`}
          </p>
        </div>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {usersLoading && users.length === 0 ? (
                ...
                filtered.map(u => (
                  <tr
                    key={safeText(u.id, `user-${Math.random()}`)}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-semibold hidden sm:table-cell">
                      ₦{safeNumber(u.balance).toLocaleString()}
                    </td>
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-border text-xs text-muted-foreground flex items-center justify-between">
            <span>
              Showing {filtered.length} of {safeNumber(usersTotal).toLocaleString()} users
            </span>

            {safeNumber(usersTotal) > users.length && (
              <span className="text-amber-400">
                Showing first {users.length} — use filters to narrow down
              </span>
            )}
          </div>
        )}
