 ...
 * financial commitments, asking for PINs/passwords, or performing
 * any account or financial action.
 */
...
export async function generateSupportReply(
  ...
  try {
    ...
    const reply =
      parsed.reply?.trim() ||
      'I\'m sorry, I was unable to generate a response. Please try again or ask to speak with a human agent.';

    const tokensUsed =
      completion.usage?.total_tokens;

    const keywordEscalation =
      ESCALATION_REGEX.test(userMessage) ||
      ESCALATION_REGEX.test(reply);

    const escalation_needed =
      !!parsed.escalation_needed ||
      keywordEscalation;
    ...
    logger.error(
      { err },
      'AI support reply generation failed',
    );

    return {
      reply:
        'I\'m having trouble connecting right now. Let me connect you with a human support agent who can help immediately.',
