...
export function AppProvider({
  ...
  /*
   * Restore authentication only once when the provider mounts.
   *
   * This is intentionally NOT dependent on refresh callbacks.
   * Otherwise setUser() changes those callbacks and can cause
   * /auth/me to run again.
   */
  useEffect(() => {
    let mounted = true;
  ...
  /*
   * After authentication, load account information in the
   * background. These requests never block login navigation.
   */
  useEffect(() => {
    if (!user) return;
  ...
  /*
   * Keep the main wallet balance synchronized with the server.
   * This allows a balance credited by Super Admin from another
   * session/device to appear automatically in the user's app.
   */
  useEffect(() => {
    if (!user) return;
