...
function HttpStatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono font-medium ${cls}`}
    >
      {code}
    </span>
  );
...
function ApiCard({
  ...
  const [togglingEnabled, setTogglingEnabled] =
    useState(false);
  const [saveError, setSaveError] = useState('');

  const color = getApiColor(config.key);

  const hasUnconfigured = config.fields.some(
    (field) => !field.value,
  );
  ...
  async function handleToggleEnabled(value: boolean) {
    ...
    try {
      await onToggleEnabled(config.key, value);
    } finally {
    ...
    <div className="bg-[#0D1F3C] rounded-2xl border border-white/[0.06] p-5 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${color.bg} border ${color.border}`}
          >
            <Cpu
      ...
      {hasUnconfigured && (
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20">
          ...
          <p className="text-xs text-amber-400">
            Some fields are not configured
          </p>
        </div>
      )}
...
// Only active/current integrations are shown here.
// ClubKonnect has intentionally been removed.
...
function TxLogRow({
  ...
  return (
    <tr className="border-b border-white/[0.06] hover:bg-white/[0.02] transition-colors">
      ...
      <td className="px-4 py-3 text-xs text-muted-foreground font-mono max-w-[160px] truncate">
        {log.endpoint}
      </td>
...
export default function APIManagement() {
  ...
  const [latencyMap, setLatencyMap] =
  ...
  // Log tabs
  const [logTab, setLogTab] =
    useState<LogTab>('error');

  const [apiFilter, setApiFilter] =
    useState('');

  // Error logs
  const [errorLogs, setErrorLogs] =
    useState<ApiLogEntry[]>([]);

  const [errorTotal, setErrorTotal] =
    useState(0);

  const [errorPages, setErrorPages] =
    useState(1);

  const [errorPage, setErrorPage] =
    useState(1);

  const [loadingErrors, setLoadingErrors] =
    useState(false);

  // Transaction logs
  const [txLogs, setTxLogs] =
    useState<ApiLogEntry[]>([]);

  const [txTotal, setTxTotal] =
    useState(0);

  const [txPages, setTxPages] =
    useState(1);

  const [txPage, setTxPage] =
    useState(1);

  const [loadingTx, setLoadingTx] =
    useState(false);
    ...
    try {
      ...
      setConfigs((previous) =>
        previous.map((config) => {
          ...
          if (result) {
            newLatency[config.key] =
              result.latency;

            return {
              ...config,
              status:
                result.status as ApiConfig['status'],
              lastChecked:
                result.checkedAt,
            };
          }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Monitor and configure third-party API integrations
              </p>
            </div>
          </div>
        ...
        <section>
          ...
          {loadingConfigs ? (
            ...
            <div className="bg-[#0D1F3C] rounded-2xl border border-white/[0.06]">
              <EmptyState
                icon={Cpu}
                title="No API configurations found"
                subtitle="API configs will appear here once set up"
              />
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  errorLogs.length === 0 ? (
                    ...
                    <EmptyState
                      icon={CheckCircle2}
                      title="No error logs"
                      subtitle="All API calls are running cleanly"
                    />
                  ) : (
                    <>
                ...
                >
                  ...
                  txLogs.length === 0 ? (
                    ...
                    <EmptyState
                      ...
                      subtitle="API transaction history will appear here"
                    />
                  ) : (
                    <>
                      <div className="overflow-x-auto">
                      ...
                      <div className="px-5 py-3 border-t border-white/[0.06] flex items-center justify-between">
                        <p className="text-xs text-muted-foreground">
                          Showing {txLogs.length}{' '}
                          of {txTotal} transaction
                          logs
                        </p>
