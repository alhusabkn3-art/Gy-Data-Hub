...
async function applyCashbackIfEligible(opts: {
  ...
  const txResult = await db.transaction(async (tx) => {
    ...
    if (insertResult.rows[0]) {
      ...
      if (!row) return null;
      if (row.wallet_txn_id !== null) return null;
    ...
    const cbBalAfter = (
      parseFloat(cbBalBefore) + cashbackAmount
    ).toFixed(2);
...
export async function transferCashbackToMain(
  ...
  const result = await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`
      UPDATE cashback_wallets
      SET balance = ${newCbBal}, updated_at = NOW()
      WHERE user_id = ${userId}::uuid
    `);

    await tx.execute(sql`
      UPDATE wallets
      SET balance = ${newMBal}, updated_at = NOW()
      WHERE user_id = ${userId}::uuid
    `);

    const ref =
      `GY-CBT-${Date.now()}-${Math.random()
        .toString(36)
        .slice(2, 6)
        .toUpperCase()}`;
...
//
// IMPORTANT:
// This lookup follows the same Super Admin pricing model used by the
// ClubKonnect data-plan endpoint.
//
// A rule can be matched by:
//   1. exact network (e.g. MTN)
//   2. exact provider (e.g. MTN)
//   3. a provider-only ClubKonnect/Nellobyte rule where network is empty.
//
// This prevents a valid Super Admin rule such as:
//   provider = ClubKonnect
//   network  = NULL
//   plan_id  = 800.01
//   selling_price = 450
//
// from being rejected during purchase.
  ...
  try {
    ...
    const priceResult = await db.execute<{
      ...
      WHERE plan_id = ${planCode}
        ...
        AND selling_price > 0
        AND (
          LOWER(REPLACE(COALESCE(network, ''), ' ', '')) =
            ${normalizedNetwork}
          OR
          LOWER(REPLACE(COALESCE(provider, ''), ' ', '')) =
            ${normalizedNetwork}
          OR
          (
            LOWER(REPLACE(COALESCE(network, ''), ' ', '')) = ''
            AND LOWER(REPLACE(COALESCE(provider, ''), ' ', ''))
              IN (
                'clubkonnect',
                'clubkonnectsystems',
                'nellobyte',
    ...
    if (
      ...
      logger.warn(
        ...
        'Invalid selling price in pricing rule'
      );

      return {
        valid: false,
        error:
          'This data plan has an invalid selling price. Please contact support.',
      };
    }
    ...
    if (
      ...
      logger.warn(
        ...
        'Invalid cost price in pricing rule'
      );

      return {
        valid: false,
        error:
          'This data plan has an invalid cost price. Please contact support.',
      };
    }

    // Allow ₦1 tolerance for frontend rounding.
    if (
      Math.abs(
        sellingPrice - submittedPrice
      ) > 1
    ) {
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (
      ...
      await createNotification(
        userId,
        {
          ...
          title:
            'Airtime Sent ✅',
          body:
            `₦${numericAmount.toLocaleString('en-NG')} of ${network.toUpperCase()} airtime was delivered to ${cleanPhone}.`,
          refId: txnId,
        }
      );
      ...
      logger.info(
        ...
        'Airtime purchase pending — awaiting vendor confirmation'
      );
      ...
      logger.warn(
        ...
        'Airtime purchase failed — wallet reversed'
      );
      ...
      await createNotification(
        userId,
        {
          ...
          body:
            `₦${numericAmount.toLocaleString('en-NG')} of ${network.toUpperCase()} airtime could not be delivered to ${cleanPhone}. Your wallet has been refunded.`,
          refId: txnId,
        }
      );
    ...
    if (
      ...
      try {
        ...
        logger.error(
          ...
          'Cashback application failed — non-fatal, purchase still succeeded'
        );
      }
      ...
      await createNotification(
        userId,
        {
          ...
          body:
            `${resolvedPlanName ?? planCode} could not be delivered to ${cleanPhone}. Your wallet has been refunded.`,
          refId: txnId,
        }
      );
...
// ── GET /api/purchase/status/:requestId ───────────────────────────────────────
router.get(
  '/status/:requestId',
  async (
    req: Request,
