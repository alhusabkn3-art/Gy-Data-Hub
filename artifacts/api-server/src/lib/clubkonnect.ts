...
export async function getDataPlans(
  ...
  // First use the normal plans request exactly as before.
  let result =
    await requestPlans(baseParams);

  // Some ClubKonnect responses can require MobileNumber.
  // If that exact condition is returned, retry once with the phone.
  const firstObject =
    asObject(result.json);
  ...
  if (
    ...
    logger.warn(
      ...
      'ClubKonnect requested MobileNumber for data plans; retrying with phone',
    );
  ...
  if (
    ...
    throw new Error(
      'ClubKonnect requires a phone number to fetch data plans.',
    );
  }
  ...
  if (
    ...
    throw new Error(
      'ClubKonnect credentials are invalid. Check CLUBKONNECT_USER_ID and CLUBKONNECT_API_KEY.',
    );
  }
