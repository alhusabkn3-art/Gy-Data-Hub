...
function collectProductObjects(
  ...
  // Prevent pathological/circular provider responses
  if (depth > 12) {
    return;
  }
  ...
  // First inspect known product containers.
  ...
  // Also inspect nested objects because provider response formats
...
export async function getDataPlans(
  ...
  /*
   * Collect all possible product objects from the entire response.
   * This intentionally does not assume:
   *
   * MOBILE_NETWORK -> MTN -> [0] -> PRODUCT
   *
   * because that assumption is what caused plans to disappear.
   */
  ...
  if (rawProducts.length === 0) {
     ...
     * Diagnostic logging.
     *
     * This is deliberately limited so API credentials are never
