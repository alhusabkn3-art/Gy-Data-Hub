 ...
 *
 * DATA + AIRTIME PROVIDER: SME API
 *
 * IMPORTANT:
 * - SME_API_KEY must stay on the server.
 * - Do NOT put the API key in frontend code.
 * - Data plan sent to SME API must be the SME API Plan ID.
 */
...
function isSameNetwork(
  ...
  const left = normalizeNetwork(a);
  const right = normalizeNetwork(b);
...
async function applyCashbackIfEligible(opts: {
  ...
  if (
    !eligibleServices.includes('data')
  ) {
    return {
      applied: false,
      amount: 0,
...
router.post(
  ...
  async (
    ...
    try {
       ...
       * IMPORTANT:
       *
       * dataPlan is the SME API Plan ID.
       *
       * Do NOT convert it to the old ClubKonnect
       * plan code.
       */
      const selectedPlan =
        String(
      ...
      logger.info(
        ...
        'SME API data purchase initiated',
      );
      ...
      /*
       * We MUST have a matching pricing rule.
       *
       * This prevents a client from inventing a price
       * or sending an arbitrary SME API Plan ID.
       */
      if (!pricing) {
        res.status(404).json({
          success: false,
          error:
            'Data plan pricing was not found. Please refresh the plans and try again.',
          planCode:
            selectedPlan,
          network,
        });
        ...
        await debitWalletAndCreateTransaction(
          {
            ...
            type: 'data',
            description:
              `Data purchase: ${resolvedPlanName} to ${cleanPhone}`,
            metadata: {
              provider:
                'smeapi',
              network,
      ...
      await createNotification(
        userId,
        {
          ...
          body:
            `${resolvedPlanName} is being sent to ${cleanPhone}.`,
          refId: txnId,
        },
      );
      ...
      try {
        vendorResult =
          await purchaseData({
             ...
             *
             * Send the actual SME API plan ID.
             *
             * Example:
             * MTN 500MB SME 7days = Plan ID 1
             *
             * Do NOT send the old ClubKonnect
             * "500.00" plan code here.
             */
            dataPlan:
              resolvedPlanCode,
            reference:
        ...
        await createNotification(
          userId,
          {
            ...
            body:
              `${resolvedPlanName} could not be delivered. Your wallet has been refunded.`,
            refId: txnId,
          },
        );
      ...
      logger.info(
        ...
        'SME API data purchase response',
      );
      ...
      if (
        ...
        await createNotification(
          userId,
          {
            ...
            body:
              `${resolvedPlanName} has been sent to ${cleanPhone}.`,
            refId: txnId,
          },
        );
      ...
      res.status(422).json({
        ...
        providerRef,
        error:
          `SME API returned: ${normalizedStatus} — ${
            vendorResult.message ||
            'No message'
          }`,
      });
...
router.post(
  ...
  async (
    ...
    try {
       ...
       * SME API applies its own provider discount.
       *
       * We therefore debit the user's requested
       * airtime amount.
       */
        ...
        await debitWalletAndCreateTransaction(
          {
            ...
            type: 'airtime',
            description:
              `Airtime purchase: ₦${confirmedAmount} to ${cleanPhone}`,
            metadata: {
              provider:
                'smeapi',
              network,
      ...
      await createNotification(
        userId,
        {
          ...
          body:
            `₦${confirmedAmount.toFixed(
              2,
            )} airtime is being sent to ${cleanPhone}.`,
          refId: txnId,
        },
      );
      ...
      try {
        ...
        await createNotification(
          userId,
          {
            ...
            body:
              `Airtime to ${cleanPhone} could not be processed. Your wallet has been refunded.`,
            refId: txnId,
          },
        );
        ...
        await createNotification(
          userId,
          {
            ...
            body:
              `₦${confirmedAmount.toFixed(
                2,
              )} airtime has been sent to ${cleanPhone}.`,
            refId: txnId,
          },
        );
      ...
      await createNotification(
        userId,
        {
          ...
          title:
            'Airtime Purchase Failed',
          body:
            `Airtime to ${cleanPhone} failed. Your wallet has been refunded.`,
          refId: txnId,
        },
      );
