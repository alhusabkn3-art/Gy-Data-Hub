...
export function AppProvider({
  ...
  /*
   * IMPORTANT:
   * Admin manual wallet funding changes the database from a different
   * browser/session. The previous frontend only loaded the wallet when
   * the user logged in or manually triggered a refresh.
   *
   * This keeps the displayed balance synchronized with /api/user/wallet.
   * It also refreshes immediately when the app becomes visible or gains
   * focus.
   */
  useEffect(() => {
    if (!user) return;
    ...
    const syncWallet = async () => {
      try {
        ...
        /*
         * Do not set balance to 0 when a network request fails.
         * The last known valid balance must remain visible.
         */
      }
    };
