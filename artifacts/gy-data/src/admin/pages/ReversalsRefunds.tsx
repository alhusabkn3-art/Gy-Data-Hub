...
function txEmoji(type: unknown): string {
...
export default function ReversalsRefunds() {
  const [reversals, setReversals] =
    useState<ReversalRecord[]>([]);
  ...
  const fetchReversals = useCallback(
    async (p: number) => {
      setLoading(true);
      setError(null);

      try {
        const data = await apiGetReversals({
          page: p,
        });

        setReversals(
          Array.isArray(data.reversals)
            ? data.reversals
            : [],
        );
        ...
        setPages(
          Math.max(1, safeNumber(data.pages, 1)),
        );
      } catch (err: unknown) {
        const msg =
  ...
  const filtered = reversals.filter(r => {
    ...
    return (
      safeSearch(r.userName).includes(q) ||
      safeSearch(r.userPhone).includes(q) ||
      safeSearch(r.txReference).includes(q) ||
      safeSearch(r.reason).includes(q) ||
      safeSearch(r.originalTransactionId).includes(q) ||
      safeSearch(r.performedByName).includes(q)
    );
  });

  const totalAmount = reversals.reduce(
    (sum, r) =>
      sum + safeNumber(r.amount),
    0,
  );

  const latestDate =
    reversals.length > 0
      ? safeDateOnly(reversals[0].createdAt)
      : 'None';
  ...
  return (
    <div className="min-h-screen bg-[#0A1628] text-white p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          onClick={() => void fetchReversals(page)}
          disabled={loading}
      ...
      {/* Error state */}
      {error && !loading && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-5 flex items-center justify-between gap-4">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                filtered.map(r => {
                  ...
                  const shortTxId =
                    originalTransactionId.length > 12
                      ? `${originalTransactionId.slice(0, 12)}…`
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-zinc-300 font-mono text-xs leading-tight">
                              {shortTxId}
                            </p>
