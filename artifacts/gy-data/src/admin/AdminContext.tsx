...
// ── Safe backend response normalizers ────────────────────────────────────────
...
const normalizeWeeklyRevenue = (value: unknown): WeeklyRevenue[] => {
  const raw = Array.isArray(value)
    ...
    : value &&
        typeof value === 'object' &&
        Array.isArray(
          (value as { revenue?: unknown }).revenue
        )
      ? (value as { revenue: unknown[] }).revenue
      : [];
...
export const AdminProvider = ({
  ...
  async function refreshStats() {
    ...
    try {
      ...
      if (res.ok) {
        setStats(normalizeStats(await res.json()));
      }
    } catch (err) {
  ...
  async function fetchWeeklyRevenue() {
    ...
    try {
      ...
      if (res.ok) {
        setWeeklyRevenue(
          normalizeWeeklyRevenue(await res.json())
        );
      }
    } catch (err) {
  ...
  async function fetchServices() {
    ...
    try {
      ...
      if (res.ok) {
        setServicesData(
          normalizeServices(await res.json())
        );
      }
    } catch (err) {
  ...
  async function fetchUsers(params?: {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUsers(list);
        setUsersTotal(
          toNumber(data.total, list.length)
        );
      }
    } catch (err) {
