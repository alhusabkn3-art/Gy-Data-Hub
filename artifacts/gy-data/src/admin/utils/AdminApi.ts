...
export async function apiGetReversals(
  ...
  const r = await adminApi(
    `/api/admin/reversals${query ? `?${query}` : ''}`
  );
...
export async function apiUpdateServiceSetting(
  ...
  const r = await adminApi(
    `/api/admin/service-settings/${serviceKey}`,
    {
      method: 'PATCH',
...
export async function apiGetUserLoginHistory(
  ...
  const r = await adminApi(
    `/api/admin/users/${userId}/login-history?page=${page}&limit=${limit}`
  );
...
export async function apiGetStaffActivities(
  staffId: string,
  page = 1,
  limit = 25
): Promise<{
  ...
  const r = await adminApi(
    `/api/admin/staff/${staffId}/activities?page=${page}&limit=${limit}`
  );
...
export async function apiGetApiLogs(
  ...
  if (params?.endpoint) {
    qs.set('endpoint', params.endpoint);
  }

  if (params?.statusCode !== undefined) {
    qs.set('statusCode', String(params.statusCode));
  }

  const query = qs.toString();

  const r = await adminApi(
    `/api/admin/api-logs${query ? `?${query}` : ''}`
  );
...
export async function apiGetFundingStats(): Promise<FundingStats> {
  const r = await adminApi(
    '/api/admin/funding-requests/stats'
  );
...
export async function apiSendUserNotification(
  userIds: string[],
  title: string,
  body: string,
  type = 'info'
