  ...
  socket.on('join:conversation', async (conversationId: string) => {
    ...
    try {
      ...
      if (sess.userId) {
        ...
        if (result.rows[0]?.customer_id === sess.userId) {
          void socket.join(`conversation:${conversationId}`);
        }
      }
    } catch (err) {
...
async function resolveStuckTransaction(
  ...
  let resolvedStatus: 'success' | 'pending' | 'failed' = 'failed';
...
function startServer() {
  try {
    ...
    httpServer.listen(port, '0.0.0.0', () => {
      ...
      // Database recovery must NEVER prevent the HTTP server from starting.
      setTimeout(() => {
